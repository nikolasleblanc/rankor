        ...
        <div className="flex flex-auto items-center">
          {officialImageSrc !== null && (
            <img
              ...
              width="50"
              height="50"
              src={officialImageSrc.replace('http:', 'https:')}
            />
          )}
          {firstName} {lastName} ({currentTeam ? abbreviation : 'NA'})
        </div>
        <div className="flex-none ma3">
          <DragHandle />
        </div>
        <div className="flex-none ma3 pointer" onClick={onRemovePlayer}>
          x
        </div>
      </div>
    ) : (
