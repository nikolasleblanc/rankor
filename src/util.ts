...
const calculateFantasyPoints = (statSet: any, playerStatType: string) => (
  playerStats: any
): number => {
  const totalPointsByStat: number[] = R.map(
    (statType: string) =>
      R.pathOr(0, [statType], playerStats) *
      R.pathOr(0, [statType], POINT_VALUES[playerStatType]),
    statSet
  );
  const totalPoints: number = totalPointsByStat.reduce(
...
export const getStatSet = (
  playerPosition: PlayerPositions,
  playerStatType: StatType
): any => {
  const relevantStats = R.intersection<any>(
    STATS[playerPosition],
    R.keys(R.prop(playerStatType, POINT_VALUES))
  );
  ...
  R.compose(
    R.tap((players: any) => {
      store.set(
        ...
        R.compose(
          R.map(R.prop('id')),
          R.sortWith([R.descend(R.prop('fantasyPoints'))])
        )(players)
      );
    }),
    R.map((player: any) => {
      ...
      return {
        fantasyPoints: calculateFantasyPoints(statSet, getStatType(position))(
          R.path(['stats', getStatType(position)], player)
        ),
...
export const setLocalStorage = (key: string, position: string, value: any) =>
  store.set(key, R.assocPath([position], value, store.get(key)));

export const setLastUpdated = (position: string) =>
  R.tap((_: any) => setLocalStorage('lastUpdated', position, new Date()));
...
export const setPlayerStatsIndexedById = (position: string) =>
  R.tap((players: any) =>
    setLocalStorage(
      'playerStatsById',
      position,
      R.indexBy(R.prop('id'), players)
    )
  );

export const setPlayersArray = (position: string) =>
  R.tap((players: any) => setLocalStorage('players', position, players));

export const setPlayerStatsArray = (position: string) =>
  R.tap((players: any) => setLocalStorage('playerStats', position, players));
...
export const getPlayerData = (position: string) => () =>
  fetch(`${API_PLAYER_URL}?position=${position}`).then(
...
export const getPlayerStatsData = (
  ...
  fetch(
    `${API_PLAYER_STATS_URL}?position=${position}&date=${date.length &&
      date
        .reverse()
        .map(i => format(i, 'YYYYMMDD'))
        .join('-')}`
...
export const movePlayerToIndex = (
  playerId: number,
  index: number,
  rank: any[]
) => {
