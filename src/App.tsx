...
export const getWeekDateRange = (week: number): Date[] => [
  addWeeks(SEASON_START_DATE, week - 1),
  addDays(addWeeks(SEASON_START_DATE, week - 1), 6)
];
...
> {
  ...
  public authenticateUser() {
    firebase
      ...
      .then((result: any) => {
        ...
        this.rankRef = getRankRef(
          getRefPath(store.get('user').uid, '/' + this.state.position)
        );
  ...
  public componentDidMount() {
    ...
    // tslint:disable-next-line
    if (R.or(R.isNil(store.get('token')), R.isNil(store.get('user')))) {
    ...
    if (store.get('user') !== undefined) {
      ...
      this.rankRef.once('value').then((snapshot: any) => {
        if (R.not(R.isNil(snapshot.val()))) {
          this.setState({
            ...this.state,
            rank: snapshot.val() !== null ? snapshot.val() : store.get('sort')
          });
        }
      });
  ...
  public loadPlayersIntoState(
    ...
    R.ifElse(
      ...
      () =>
        Promise.all([
          ...
          store.get('user') !== undefined
            ? getRankRef(
                getRefPath(store.get('user').uid, '/' + position)
              ).once('value')
    ...
    )(R.prop(position, store.get('lastUpdated'))).then((response: any) => {
      ...
      if (store.get('user') !== undefined) {
        getRankRef(
          ...
          this.setState({
            ...this.state,
            rank: snapshot.val() !== null ? snapshot.val() : rank
          });
        });
      }
    });
  ...
  public render() {
    ...
    const playersById = R.propOr(
      {},
      this.state.position,
      store.get('playersById')
    );
    const playerStatsById = R.propOr(
      {},
      this.state.position,
    ...
    return (
      <div className="flex flex-column vh-100">
        ...
        >
          <PlayerDetail
            player={playersById[this.state.playerSelected] || {}}
            stats={playerStatsById[this.state.playerSelected]}
            onClose={this.closePlayerPage}
        ...
        <div className="flex-none header">
          ...
          <div className="flex justify-between ma2">
            <div>
              <button
                onClick={this.changePosition('QB')}
                className="bg-transparent ba b--none underline"
              >
                QB
              </button>
              <button
                onClick={this.changePosition('RB')}
                className="bg-transparent ba b--none underline"
              >
                RB
              </button>
              <button
                onClick={this.changePosition('TE')}
                className="bg-transparent ba b--none underline"
              >
                TE
              </button>
              <button
                onClick={this.changePosition('WR')}
                className="bg-transparent ba b--none underline"
              >
                WR
              </button>
            ...
            <div>
              <button
                onClick={this.clearRanks}
                className="bg-transparent ba b--none underline"
              >
                Clear Week {this.state.week} {this.state.position} Ranks
              </button>
            </div>
            <div>
              {!this.state.loggedIn ? (
          ...
          <div className="ma2">
            <Select
              ...
              options={R.drop(25, this.state.rank).map((playerId: number) => ({
                label:
                  R.path([playerId, 'firstName'], playersById) +
                  ' ' +
                  R.path([playerId, 'lastName'], playersById),
                value: playerId
              }))}
